...
def color_rec (image_path):
    ...
    denoised_image = cv2.fastNlMeansDenoisingColored(img, None, h=10, templateWindowSize=7,searchWindowSize=21)
    #cv2.imshow('bruh', denoised_image)
    #cv2.waitKey(0)
    ...
    img_hsv = cv2.cvtColor(denoised_image, cv2.COLOR_BGR2HSV)
    file_name = f'./dummy/{Path(image_path).stem}_1original{Path(image_path).suffix}'
    ...
    k = 10
    retval, labels, centers = cv2.kmeans(pixels, k, None, criteria, 20, cv2.KMEANS_RANDOM_CENTERS)

    # Convert back to 8-bit values
    centers = np.uint8(centers)
    segmented_data = centers[labels.flatten()]
    segmented_image = segmented_data.reshape(img_hsv.shape)
    file_name = f'./dummy/{Path(image_path).stem}_segmented{Path(image_path).suffix}'
    #cv2.imwrite(file_name, cv2.cvtColor(segmented_image, cv2.COLOR_HSV2RGB))
    ...
    file_name = f'./dummy/bro{Path(image_path).stem}_mask1{Path(image_path).suffix}'
    cv2.imwrite(file_name, mask1)
    file_name = f'./dummy/bro{Path(image_path).stem}_mask1_color{Path(image_path).suffix}'
    ...
    file_name = f'./dummy/bro{Path(image_path).stem}_mask2_color{Path(image_path).suffix}'
    cv2.imwrite(file_name, result)
    plt.tight_layout()
    plt.show()
...
print(color_rec('../Training Data/Select Cropped/image8cropped1.jpg'))